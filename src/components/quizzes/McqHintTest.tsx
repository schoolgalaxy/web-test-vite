import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import '../../assets/css/McqTest.css'; // Reusing MCQ test styles

interface AnswerOption {
  text: string;
  isCorrect: boolean;
  rationale: string;
}

interface Question {
  questionNumber: number;
  question: string;
  answerOptions: AnswerOption[];
  hint?: string;
}

interface QuizData {
  unique_id: string;
  title: string;
  description: string;
  profile_pic_link: string;
  type: string;
  version: number;
  questions: Question[];
}

interface McqHintTestProps {
  quizCategory: string;
  routePrefix: string;
}

const McqHintTest: React.FC<McqHintTestProps> = ({ quizCategory, routePrefix }) => {
  const { quizId } = useParams<{ quizId: string }>();
  const navigate = useNavigate();
  const [quizData, setQuizData] = useState<QuizData | null>(null);
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [score, setScore] = useState(0);
  const [showScore, setShowScore] = useState(false);
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [isCorrectAnswer, setIsCorrectAnswer] = useState<boolean | null>(null);
  const [answered, setAnswered] = useState(false);
  const [showHint, setShowHint] = useState(false);

  useEffect(() => {
    const loadQuizData = () => {
      try {
        const modules = import.meta.glob('/src/assets/json_data/**/*.json', { eager: true });
        const targetPathSuffix = `/${quizCategory}/${quizId}.json`;
        const matchKey = Object.keys(modules).find((key) => key.endsWith(targetPathSuffix));

        if (!matchKey) {
          throw new Error(`Quiz data not found for ${quizCategory}/${quizId}`);
        }

        const module: any = modules[matchKey as keyof typeof modules];
        const data: QuizData = module.default ?? module;

        // Verify this is a MCQ_10_Hint type quiz
        if (data.type !== 'MCQ_10_Hint') {
          throw new Error(`Quiz ${quizId} is not a MCQ_10_Hint type quiz`);
        }

        setQuizData(data);
      } catch (error) {
        console.error(`Error loading ${quizCategory} quiz data:`, error);
        navigate('/error');
      }
    };

    if (quizId && quizCategory) {
      loadQuizData();
    }
  }, [quizId, quizCategory, navigate]);

  if (!quizData) {
    return <div>Loading quiz...</div>;
  }

  const handleAnswerOptionClick = (optionText: string) => {
    if (selectedAnswer === optionText && !isCorrectAnswer) {
      return;
    }

    setSelectedAnswer(optionText);
    const currentQuestion = quizData.questions[currentQuestionIndex];
    const selectedOption = currentQuestion.answerOptions.find(opt => opt.text === optionText);
    const correct = selectedOption?.isCorrect || false;
    setIsCorrectAnswer(correct);

    if (correct) {
      setScore(score + 1);
      setAnswered(true);
    } else {
      setAnswered(false);
    }
  };

  const handleNextQuestion = () => {
    const nextQuestion = currentQuestionIndex + 1;
    if (nextQuestion < quizData.questions.length) {
      setCurrentQuestionIndex(nextQuestion);
      setSelectedAnswer(null);
      setIsCorrectAnswer(null);
      setAnswered(false);
      setShowHint(false);
    } else {
      setShowScore(true);
    }
  };

  return (
    <div className="mcq-test-container">
      <button onClick={() => navigate(routePrefix)} className="back-button">Back to {quizData.title} Quizzes</button>
      {showScore ? (
        <div className="score-section">
          <h2>Congratulations! You've completed the {quizData.title} quiz.</h2>
          <p>You scored {score} out of {quizData.questions.length}</p>
          <button onClick={() => navigate(routePrefix)}>Take Another Quiz</button>
        </div>
      ) : (
        <>
          <div className="question-section">
            <div className="question-count">
              <span>Question {currentQuestionIndex + 1}</span>/{quizData.questions.length}
            </div>
            <div className="question-text">{quizData.questions[currentQuestionIndex].question}</div>
            {quizData.questions[currentQuestionIndex].hint && (
              <div className="hint-section">
                {!showHint ? (
                  <button
                    className="hint-button"
                    onClick={() => setShowHint(true)}
                  >
                    Show Hint
                  </button>
                ) : (
                  <div className="hint-text">
                    <strong>Hint:</strong> {quizData.questions[currentQuestionIndex].hint}
                  </div>
                )}
              </div>
            )}
          </div>
          <div className="answer-options">
            {quizData.questions[currentQuestionIndex].answerOptions.map((option, index: number) => {
              const isSelected = selectedAnswer === option.text;
              let buttonClass = "answer-button";

              if (isSelected) {
                if (isCorrectAnswer === true) {
                  buttonClass += " correct";
                } else if (isCorrectAnswer === false) {
                  buttonClass += " incorrect";
                }
              }

              return (
                <button
                  key={index}
                  className={buttonClass}
                  onClick={() => handleAnswerOptionClick(option.text)}
                  disabled={answered}
                >
                  {option.text}
                </button>
              );
            })}
          </div>
          {selectedAnswer !== null && (
            <div className="feedback-section">
              {isCorrectAnswer ? (
                <>
                  <p className="feedback-correct">Correct!</p>
                  {(() => {
                    const currentQuestion = quizData.questions[currentQuestionIndex];
                    const selectedOption = currentQuestion.answerOptions.find(opt => opt.text === selectedAnswer);
                    return selectedOption?.rationale && (
                      <div className="rationale">
                        <strong>Explanation:</strong> {selectedOption.rationale}
                      </div>
                    );
                  })()}
                  <button onClick={handleNextQuestion} className="next-question-button">
                    Next Question
                  </button>
                </>
              ) : (
                <>
                  <p className="feedback-incorrect">Incorrect. Please try again.</p>
                  {(() => {
                    const currentQuestion = quizData.questions[currentQuestionIndex];
                    const selectedOption = currentQuestion.answerOptions.find(opt => opt.text === selectedAnswer);
                    return selectedOption?.rationale && (
                      <div className="rationale">
                        <strong>Explanation:</strong> {selectedOption.rationale}
                      </div>
                    );
                  })()}
                </>
              )}
            </div>
          )}
        </>
      )}
    </div>
  );
};

export default McqHintTest;